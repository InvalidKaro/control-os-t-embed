...
def check_platformio(errors: list[str]) -> None:
    ...
    if not any(flag in text for flag in FASTLED_DRIVER_FLAGS):
        errors.append(
            "missing FastLED ESP32 driver flag: expected one of "
            + ", ".join(FASTLED_DRIVER_FLAGS)
        )
    ...
    if "FastLED.git#3.9.15" not in text and "FastLED@3.9.15" not in text:
        errors.append("FastLED must be pinned to 3.9.15 for this release candidate")
...
def check_sources(errors: list[str]) -> None:
    source_files = list((ROOT / "src").glob("*.cpp"))
    source_files += list((ROOT / "include").glob("*.h"))
    ...
    for path in source_files:
        ...
        for pattern in FORBIDDEN_SOURCE_PATTERNS:
            if re.search(pattern, text, flags=re.IGNORECASE | re.DOTALL):
                errors.append(
                    f"forbidden/unfinished source pattern {pattern!r} "
...
def main() -> int:
    ...
    print("- FastLED 3.9.15 release pin present")
